...
param(
    ...
    [string] $CompletionIncrementalReusePatch,
    [string] $WorkRoot,
    [string] $OutputRoot,
...
$ExpectedCurrentServiceThirdPartyZipSha256 = 'b2fa503dbd7efee818bcd94443f8e30a65e0fd4a85e2169b8e99bbc630fa7893'
...
$ExpectedCompletionIncrementalReusePatchSha256 = '83f4f2421670e75deeecebab8d17a2517485e3811ecfc754b415e9a485591b80'
$ExpectedPreparationBaselineDistributionId = 'roslyn-3aeb96c9-systemexplorer-17827506d20d-win-x64-v3'
...
$RuntimeArchiveName = 'roslyn-completion-incremental-reuse-server.zip'
$EvidenceFileName = 'CompletionIncrementalReuseProductionRuntimeEvidence.json'
$AdoptionValuesFileName = 'ServiceRuntimeAdoptionValues.txt'
$OwnershipMarkerName = '.systemexplorer-completion-incremental-reuse-production-owned'
...
function Show-Usage {
    Write-Host 'Usage:'
    Write-Host '  Build-ProductionCompletionIncrementalReuseRuntime.cmd -RoslynRepositoryRoot <path> -CurrentServiceThirdPartyZip <path> -CompletionIncrementalReusePatch <path> [-WorkRoot <path>] [-OutputRoot <path>] [-DotNetCacheRoot <path>] [-KeepArtifacts]'
    Write-Host ''
    ...
    Write-Host '  SYSTEMEXPLORER_COMPLETION_INCREMENTAL_REUSE_PATCH'
...
if ([string]::IsNullOrWhiteSpace($CompletionIncrementalReusePatch)) { $CompletionIncrementalReusePatch = $env:SYSTEMEXPLORER_COMPLETION_INCREMENTAL_REUSE_PATCH }
if ([string]::IsNullOrWhiteSpace($RoslynRepositoryRoot) -or [string]::IsNullOrWhiteSpace($CurrentServiceThirdPartyZip) -or [string]::IsNullOrWhiteSpace($CompletionIncrementalReusePatch)) {
    [Console]::Error.WriteLine('Roslyn repository, current V3 Service.ThirdParty.zip, and canonical 0004 patch are required via explicit parameters or supported environment variables.')
    Show-Usage
    exit $InvalidArgumentsExitCode
}
...
try {
    ...
    $CompletionIncrementalReusePatch = Get-NormalizedPath $CompletionIncrementalReusePatch
    $shortBaseRoot = Join-Path $env:SystemDrive 'Temp/SECR4'
    ...
    if (-not (Test-Path -LiteralPath $CompletionIncrementalReusePatch -PathType Leaf)) { Fail-Closed "CompletionIncrementalReusePatch does not exist: $CompletionIncrementalReusePatch" }
    ...
    Write-BuildLog "Completion incremental-reuse production runtime build started. RoslynRepositoryRoot=$RoslynRepositoryRoot"
    Write-BuildLog "CurrentServiceThirdPartyZip=$CurrentServiceThirdPartyZip"
    Write-BuildLog "CompletionIncrementalReusePatch=$CompletionIncrementalReusePatch"
    ...
    if ($currentThirdPartyHash -ne $ExpectedCurrentServiceThirdPartyZipSha256) {
        Fail-Closed "current V3 Service.ThirdParty.zip SHA-256 mismatch; expected=$ExpectedCurrentServiceThirdPartyZipSha256 actual=$currentThirdPartyHash"
    }
    Write-Host "Verified current V3 Service.ThirdParty.zip SHA-256: $currentThirdPartyHash"

    $completionIncrementalReusePatchSha256 = Get-FileSha256Lower $CompletionIncrementalReusePatch
    ...
    $distributionId = "roslyn-3aeb96c9-systemexplorer-$($completionIncrementalReusePatchSha256.Substring(0, 12))-win-x64-v4"
    Write-Host "Canonical 0004 SHA-256: $completionIncrementalReusePatchSha256"
    Write-Host "Production distribution id: $distributionId"

    Assert-UnifiedDiffHunkCounts $CompletionIncrementalReusePatch
    $patchText = [IO.File]::ReadAllText($CompletionIncrementalReusePatch)
    foreach ($requiredText in @(
        'WithDoNotCreateCreationPolicyForCompletion',
        'WithFrozenPartialCompilationsForCompletion',
        '_documentIdToFrozenSolutionForCompletion',
        'WithFrozenPartialSemanticsForCompletionCachesPerImmutableSolution',
        'WithFrozenPartialSemanticsForCompletionPreservesWarmIncrementalProjectState',
        'WithFrozenPartialSemanticsForCompletionPreservesWarmSameLanguageDependencyState',
        'GettingCompletionListAfterWarmSourceEditDoesNotRerunSourceGenerator')) {
        if (-not $patchText.Contains($requiredText)) { Fail-Closed "canonical 0004 is missing required production/regression content: $requiredText" }
    }
    ...
    [void](Invoke-GitText $OwnedWorktree @('apply', '--check', '--', $CompletionIncrementalReusePatch))
    [void](Invoke-GitText $OwnedWorktree @('apply', '--', $CompletionIncrementalReusePatch))
    [void](Invoke-GitText $OwnedWorktree @('diff', '--check'))
    Write-Host 'Canonical 0001 -> 0002 -> 0003 -> 0004 patch chain applies cleanly.'
    ...
    Copy-Item -LiteralPath $CompletionIncrementalReusePatch -Destination (Join-Path $patchesRoot $CanonicalCompletionIncrementalReusePatchName)
    ...
    if ($staged0004Hash -ne $completionIncrementalReusePatchSha256) { Fail-Closed 'staged 0004 bytes are not byte-identical to the source build-input copy.' }
...
SystemExplorer patched private Roslyn Language Server win-x64 v4
...
LICENSE.txt is preserved byte-for-byte from the verified current V3 Service.ThirdParty.zip and corresponds to pinned Roslyn repository root License.txt (Git blob $ExpectedLicenseBlob).
LICENSE.txt SHA-256:
$($licenseSha256.ToUpperInvariant())

ThirdParty notices provenance:
ThirdPartyNotices.rtf is preserved byte-for-byte from the verified current V3 Service.ThirdParty.zip and corresponds to src/NuGet/ThirdPartyNotices.rtf at the pinned Roslyn commit (Git blob $ExpectedThirdPartyNoticesBlob).
ThirdPartyNotices.rtf SHA-256:
$($thirdPartyNoticesSha256.ToUpperInvariant())

This is a private SystemExplorer production build from the pinned Roslyn source commit plus canonical 0001, canonical 0002, canonical 0003, and canonical 0004.
...
"@
    ...
    $newThirdPartyZipPath = Join-Path $FinalOutputRoot 'Service.ThirdParty_V4.zip'
    ...
    try {
        ...
        if ((Get-ZipEntrySha256Lower $verifyArchive $CanonicalCompletionIncrementalReusePatchThirdPartyEntry) -ne $completionIncrementalReusePatchSha256) { Fail-Closed 'generated ThirdParty 0004 SHA mismatch.' }
...
ServiceThirdPartyV4ZipSha256=$($newThirdPartyZipSha256.ToUpperInvariant())
"@
    ...
    Write-Host "  Service.ThirdParty_V4.zip: $newThirdPartyZipPath"
    ...
    Write-Host "  Service.ThirdParty_V4.zip SHA-256: $newThirdPartyZipSha256"
