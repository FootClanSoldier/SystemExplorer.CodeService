...
param(
    ...
    [string] $TypeReceiverSemanticOriginPatch,
    [string] $WorkRoot,
    [string] $OutputRoot,
...
$ExpectedCurrentServiceThirdPartyZipSha256 = 'dc00bf7b49b2a22783983238ca471a5cda8caf58cd6e82f55535707843896830'
...
$ExpectedPreparationBaselineDistributionId = 'roslyn-3aeb96c9-systemexplorer-cd4f905c4b2b-win-x64-v7'
...
$RuntimeArchiveName = 'roslyn-type-receiver-semantic-origin-server.zip'
$EvidenceFileName = 'TypeReceiverSemanticOriginProductionRuntimeEvidence.json'
$AdoptionValuesFileName = 'ServiceRuntimeAdoptionValues.txt'
$CompletionTestResultFileName = 'CompletionServiceTestsResult.txt'
$OwnershipMarkerName = '.systemexplorer-type-receiver-semantic-origin-production-owned'
...
function Show-Usage {
    Write-Host 'Usage:'
    Write-Host '  Build-ProductionTypeReceiverSemanticOriginRuntime_v2.cmd -RoslynRepositoryRoot <path> -CurrentServiceThirdPartyZip <path> -TypeReceiverSemanticOriginPatch <path> [-WorkRoot <path>] [-OutputRoot <path>] [-DotNetCacheRoot <path>] [-KeepArtifacts]'
    Write-Host ''
    ...
    Write-Host '  SYSTEMEXPLORER_TYPE_RECEIVER_SEMANTIC_ORIGIN_PATCH'
...
if ([string]::IsNullOrWhiteSpace($TypeReceiverSemanticOriginPatch)) { $TypeReceiverSemanticOriginPatch = $env:SYSTEMEXPLORER_TYPE_RECEIVER_SEMANTIC_ORIGIN_PATCH }
if ([string]::IsNullOrWhiteSpace($RoslynRepositoryRoot) -or [string]::IsNullOrWhiteSpace($CurrentServiceThirdPartyZip) -or [string]::IsNullOrWhiteSpace($TypeReceiverSemanticOriginPatch)) {
    [Console]::Error.WriteLine('Roslyn repository, current V7 Service.ThirdParty.zip, and canonical 0008 patch are required via explicit parameters or supported environment variables.')
    Show-Usage
    exit $InvalidArgumentsExitCode
}
...
try {
    ...
    $TypeReceiverSemanticOriginPatch = Get-NormalizedPath $TypeReceiverSemanticOriginPatch
    $shortBaseRoot = Join-Path $env:SystemDrive 'Temp/SECR8'
    ...
    if (-not (Test-Path -LiteralPath $TypeReceiverSemanticOriginPatch -PathType Leaf)) { Fail-Closed "TypeReceiverSemanticOriginPatch does not exist: $TypeReceiverSemanticOriginPatch" }
    ...
    Write-BuildLog "Type-receiver semantic-origin production runtime build started. RoslynRepositoryRoot=$RoslynRepositoryRoot"
    Write-BuildLog "CurrentServiceThirdPartyZip=$CurrentServiceThirdPartyZip"
    Write-BuildLog "TypeReceiverSemanticOriginPatch=$TypeReceiverSemanticOriginPatch"
    ...
    if ($currentThirdPartyHash -ne $ExpectedCurrentServiceThirdPartyZipSha256) {
        Fail-Closed "current V7 Service.ThirdParty.zip SHA-256 mismatch; expected=$ExpectedCurrentServiceThirdPartyZipSha256 actual=$currentThirdPartyHash"
    }
    Write-Host "Verified current V7 Service.ThirdParty.zip SHA-256: $currentThirdPartyHash"

    $typeReceiverSemanticOriginPatchSha256 = Get-FileSha256Lower $TypeReceiverSemanticOriginPatch
    ...
    $distributionId = "roslyn-3aeb96c9-systemexplorer-$($typeReceiverSemanticOriginPatchSha256.Substring(0, 12))-win-x64-v8"
    Write-Host "Canonical 0008 SHA-256: $typeReceiverSemanticOriginPatchSha256"
    Write-Host "Production distribution id: $distributionId"

    Assert-UnifiedDiffHunkCounts $TypeReceiverSemanticOriginPatch
    $patchText = [IO.File]::ReadAllText($TypeReceiverSemanticOriginPatch)
    $expected0008TouchedPaths = @(
    ...
    $actual0008TouchedPaths = @(
        [Text.RegularExpressions.Regex]::Matches($patchText, '(?m)^diff --git a/(.+?) b/(.+?)\r?$') |
            ForEach-Object { $_.Groups[2].Value } |
            Sort-Object -Unique
    )
    $expected0008TouchedPaths = @($expected0008TouchedPaths | Sort-Object)
    if ($actual0008TouchedPaths.Count -ne $expected0008TouchedPaths.Count -or
        @(Compare-Object -ReferenceObject $expected0008TouchedPaths -DifferenceObject $actual0008TouchedPaths).Count -ne 0) {
        Fail-Closed "canonical 0008 touched-path set is not exactly the approved two files. actual=$($actual0008TouchedPaths -join ', ')"
    }
    foreach ($requiredText in @(
        ...
        'AssertSystemExplorerCompletionEvidenceWithoutDepth')) {
        if (-not $patchText.Contains($requiredText)) { Fail-Closed "canonical 0008 is missing required type-receiver/regression content: $requiredText" }
    }
    foreach ($forbiddenText in @(
        ...
        'PROVENANCE.txt',
        '.IsStatic')) {
        if ($patchText.Contains($forbiddenText)) { Fail-Closed "canonical 0008 contains forbidden scope/content: $forbiddenText" }
    }
    ...
    [void](Invoke-GitText $OwnedWorktree @('apply', '--check', '--whitespace=error-all', '--', $TypeReceiverSemanticOriginPatch))
    [void](Invoke-GitText $OwnedWorktree @('apply', '--whitespace=error-all', '--', $TypeReceiverSemanticOriginPatch))
    [void](Invoke-GitText $OwnedWorktree @('diff', '--check'))
    Write-Host 'Canonical 0001 -> 0002 -> 0003 -> 0004 -> 0005 -> 0006 -> 0007 -> 0008 patch chain applies cleanly.'
    ...
    foreach ($requiredReceiverHelper in @(
        ...
        'ResolveNamedTypeAuthority(receiverSymbolInfo.Symbol)',
        'CandidateSymbols.Any(IsNamespaceOrTypeAuthority)',
        'GetTypeInfo(receiverExpression, cancellationToken)',
        'INamedTypeSymbol type when type.TypeKind != TypeKind.Error',
        'IAliasSymbol { Target: INamedTypeSymbol target } when target.TypeKind != TypeKind.Error',
        ...
        if (-not $helperText.Contains($requiredReceiverHelper)) { Fail-Closed "0008 type-receiver helper contract is missing: $requiredReceiverHelper" }
    }

    $resolvedTypeAuthorityIndex = $helperText.IndexOf('ResolveNamedTypeAuthority(receiverSymbolInfo.Symbol)', [StringComparison]::Ordinal)
    $namespaceOrTypeRejectIndex = $helperText.IndexOf('if (IsNamespaceOrTypeAuthority(receiverSymbolInfo.Symbol)', [StringComparison]::Ordinal)
    $ordinaryTypeInfoIndex = $helperText.IndexOf('context.SemanticModel.GetTypeInfo(', [StringComparison]::Ordinal)
    if ($resolvedTypeAuthorityIndex -lt 0 -or $namespaceOrTypeRejectIndex -lt 0 -or $ordinaryTypeInfoIndex -lt 0 -or
        $resolvedTypeAuthorityIndex -ge $namespaceOrTypeRejectIndex -or $namespaceOrTypeRejectIndex -ge $ordinaryTypeInfoIndex) {
        Fail-Closed '0008 receiver resolution ordering is invalid; named-type authority must be accepted before namespace/type fail-closed rejection and ordinary value TypeInfo fallback.'
    }

    foreach ($wireKey in @('SystemExplorer.CompletionSemanticOrigin', 'SystemExplorer.CompletionInheritanceDepth')) {
        if (-not $helperText.Contains($wireKey)) { Fail-Closed "0008 changed or removed a private semantic-origin property key: $wireKey" }
    }
    ...
    if (-not (Test-Path -LiteralPath $symbolProviderPath -PathType Leaf)) { Fail-Closed '0007-patched AbstractSymbolCompletionProvider.cs is missing after 0008.' }
    ...
    foreach ($requiredProviderText in @(
        ...
        if (-not $symbolProviderText.Contains($requiredProviderText)) { Fail-Closed "0007 request-local receiver cache/provider wiring regressed after 0008: $requiredProviderText" }
    }
    ...
    if (-not (Test-Path -LiteralPath $completionServiceTestsSourcePath -PathType Leaf)) { Fail-Closed '0008 CompletionServiceTests.cs is missing.' }
    ...
    foreach ($testName in @(
        ...
        'CompletionSemanticOriginUsesNamedTypeAliasReceiverForStaticMemberAccess',
        'CompletionSemanticOriginPreservesDeclarationAuthorityForNamespaceReceiver')) {
        if (-not $completionServiceTestsSourceText.Contains($testName)) { Fail-Closed "0007/0008 completion regression test source is missing: $testName" }
    }
    if (-not $completionServiceTestsSourceText.Contains('AssertSystemExplorerCompletionEvidenceWithoutDepth')) {
        Fail-Closed '0008 namespace fail-closed no-depth assertion helper is missing.'
    }
    ...
    Copy-Item -LiteralPath $TypeReceiverSemanticOriginPatch -Destination (Join-Path $patchesRoot $CanonicalTypeReceiverSemanticOriginPatchName)
    ...
    if ($staged0008Hash -ne $typeReceiverSemanticOriginPatchSha256) { Fail-Closed 'staged 0008 bytes are not byte-identical to the source build-input copy.' }
...
SystemExplorer patched private Roslyn Language Server win-x64 v8
...
LICENSE.txt is preserved byte-for-byte from the verified current V7 Service.ThirdParty.zip and corresponds to pinned Roslyn repository root License.txt (Git blob $ExpectedLicenseBlob).
LICENSE.txt SHA-256:
$($licenseSha256.ToUpperInvariant())

ThirdParty notices provenance:
ThirdPartyNotices.rtf is preserved byte-for-byte from the verified current V7 Service.ThirdParty.zip and corresponds to src/NuGet/ThirdPartyNotices.rtf at the pinned Roslyn commit (Git blob $ExpectedThirdPartyNoticesBlob).
ThirdPartyNotices.rtf SHA-256:
$($thirdPartyNoticesSha256.ToUpperInvariant())

This is a private SystemExplorer production build from the pinned Roslyn source commit plus canonical 0001, canonical 0002, canonical 0003, canonical 0004, canonical 0005, canonical 0006, canonical 0007, and canonical 0008.
...
"@
    ...
    $newThirdPartyZipPath = Join-Path $FinalOutputRoot 'Service.ThirdParty_V8.zip'
    ...
    try {
        ...
        if ((Get-ZipEntrySha256Lower $verifyArchive $CanonicalTypeReceiverSemanticOriginPatchThirdPartyEntry) -ne $typeReceiverSemanticOriginPatchSha256) { Fail-Closed 'generated ThirdParty 0008 SHA mismatch.' }
...
ServiceThirdPartyV8ZipSha256=$($newThirdPartyZipSha256.ToUpperInvariant())
"@
...
"@
    ...
    Write-Host "  Service.ThirdParty_V8.zip: $newThirdPartyZipPath"
    ...
    Write-Host "  Service.ThirdParty_V8.zip SHA-256: $newThirdPartyZipSha256"
