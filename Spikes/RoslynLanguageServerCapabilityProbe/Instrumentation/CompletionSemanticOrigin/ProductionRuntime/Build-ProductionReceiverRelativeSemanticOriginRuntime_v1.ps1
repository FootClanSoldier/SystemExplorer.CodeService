...
param(
    ...
    [string] $ReceiverRelativeSemanticOriginPatch,
    [string] $WorkRoot,
    [string] $OutputRoot,
...
$ExpectedCurrentServiceThirdPartyZipSha256 = 'd3fa03c662af9d2f981b84ecfadae2d79115561ba88d48a813958b2c23e507bb'
...
$ExpectedPreparationBaselineDistributionId = 'roslyn-3aeb96c9-systemexplorer-8dd66da05d85-win-x64-v6'
...
$RuntimeArchiveName = 'roslyn-receiver-relative-semantic-origin-server.zip'
$EvidenceFileName = 'ReceiverRelativeSemanticOriginProductionRuntimeEvidence.json'
$AdoptionValuesFileName = 'ServiceRuntimeAdoptionValues.txt'
$OwnershipMarkerName = '.systemexplorer-receiver-relative-semantic-origin-production-owned'
...
function Show-Usage {
    Write-Host 'Usage:'
    Write-Host '  Build-ProductionReceiverRelativeSemanticOriginRuntime_v1.cmd -RoslynRepositoryRoot <path> -CurrentServiceThirdPartyZip <path> -ReceiverRelativeSemanticOriginPatch <path> [-WorkRoot <path>] [-OutputRoot <path>] [-DotNetCacheRoot <path>] [-KeepArtifacts]'
    Write-Host ''
    ...
    Write-Host '  SYSTEMEXPLORER_RECEIVER_RELATIVE_SEMANTIC_ORIGIN_PATCH'
...
if ([string]::IsNullOrWhiteSpace($ReceiverRelativeSemanticOriginPatch)) { $ReceiverRelativeSemanticOriginPatch = $env:SYSTEMEXPLORER_RECEIVER_RELATIVE_SEMANTIC_ORIGIN_PATCH }
if ([string]::IsNullOrWhiteSpace($RoslynRepositoryRoot) -or [string]::IsNullOrWhiteSpace($CurrentServiceThirdPartyZip) -or [string]::IsNullOrWhiteSpace($ReceiverRelativeSemanticOriginPatch)) {
    [Console]::Error.WriteLine('Roslyn repository, current V6 Service.ThirdParty.zip, and canonical 0007 patch are required via explicit parameters or supported environment variables.')
    Show-Usage
    exit $InvalidArgumentsExitCode
}
...
try {
    ...
    $ReceiverRelativeSemanticOriginPatch = Get-NormalizedPath $ReceiverRelativeSemanticOriginPatch
    $shortBaseRoot = Join-Path $env:SystemDrive 'Temp/SECR7'
    ...
    if (-not (Test-Path -LiteralPath $ReceiverRelativeSemanticOriginPatch -PathType Leaf)) { Fail-Closed "ReceiverRelativeSemanticOriginPatch does not exist: $ReceiverRelativeSemanticOriginPatch" }
    ...
    Write-BuildLog "Receiver-relative semantic-origin production runtime build started. RoslynRepositoryRoot=$RoslynRepositoryRoot"
    Write-BuildLog "CurrentServiceThirdPartyZip=$CurrentServiceThirdPartyZip"
    Write-BuildLog "ReceiverRelativeSemanticOriginPatch=$ReceiverRelativeSemanticOriginPatch"
    ...
    if ($currentThirdPartyHash -ne $ExpectedCurrentServiceThirdPartyZipSha256) {
        Fail-Closed "current V6 Service.ThirdParty.zip SHA-256 mismatch; expected=$ExpectedCurrentServiceThirdPartyZipSha256 actual=$currentThirdPartyHash"
    }
    Write-Host "Verified current V6 Service.ThirdParty.zip SHA-256: $currentThirdPartyHash"

    $receiverRelativeSemanticOriginPatchSha256 = Get-FileSha256Lower $ReceiverRelativeSemanticOriginPatch
    ...
    $distributionId = "roslyn-3aeb96c9-systemexplorer-$($receiverRelativeSemanticOriginPatchSha256.Substring(0, 12))-win-x64-v7"
    Write-Host "Canonical 0007 SHA-256: $receiverRelativeSemanticOriginPatchSha256"
    Write-Host "Production distribution id: $distributionId"

    Assert-UnifiedDiffHunkCounts $ReceiverRelativeSemanticOriginPatch
    $patchText = [IO.File]::ReadAllText($ReceiverRelativeSemanticOriginPatch)
    $expected0007TouchedPaths = @(
    ...
    $actual0007TouchedPaths = @(
        [Text.RegularExpressions.Regex]::Matches($patchText, '(?m)^diff --git a/(.+?) b/(.+?)\r?$') |
            ForEach-Object { $_.Groups[2].Value } |
            Sort-Object -Unique
    )
    $expected0007TouchedPaths = @($expected0007TouchedPaths | Sort-Object)
    if ($actual0007TouchedPaths.Count -ne $expected0007TouchedPaths.Count -or
        @(Compare-Object -ReferenceObject $expected0007TouchedPaths -DifferenceObject $actual0007TouchedPaths).Count -ne 0) {
        Fail-Closed "canonical 0007 touched-path set is not exactly the approved three files. actual=$($actual0007TouchedPaths -join ', ')"
    }
    foreach ($requiredText in @(
        ...
        if (-not $patchText.Contains($requiredText)) { Fail-Closed "canonical 0007 is missing required receiver-relative/regression content: $requiredText" }
    }
    foreach ($forbiddenText in @(
        'TypeImportCompletionCacheEntry.cs',
        'src/LanguageServer/',
        'CompletionService.cs',
        'AbstractPullDiagnosticHandler.cs',
        'PROVENANCE.txt')) {
        if ($patchText.Contains($forbiddenText)) { Fail-Closed "canonical 0007 contains forbidden scope/content: $forbiddenText" }
    }
    ...
    [void](Invoke-GitText $OwnedWorktree @('apply', '--check', '--whitespace=error-all', '--', $ReceiverRelativeSemanticOriginPatch))
    [void](Invoke-GitText $OwnedWorktree @('apply', '--whitespace=error-all', '--', $ReceiverRelativeSemanticOriginPatch))
    [void](Invoke-GitText $OwnedWorktree @('diff', '--check'))
    Write-Host 'Canonical 0001 -> 0002 -> 0003 -> 0004 -> 0005 -> 0006 -> 0007 patch chain applies cleanly.'
    ...
    foreach ($requiredReceiverHelper in @(
        ...
        if (-not $helperText.Contains($requiredReceiverHelper)) { Fail-Closed "0007 receiver-relative helper contract is missing: $requiredReceiverHelper" }
    }
    foreach ($wireKey in @('SystemExplorer.CompletionSemanticOrigin', 'SystemExplorer.CompletionInheritanceDepth')) {
        if (-not $helperText.Contains($wireKey)) { Fail-Closed "0007 changed or removed a private semantic-origin property key: $wireKey" }
    }
    ...
    if (-not (Test-Path -LiteralPath $symbolProviderPath -PathType Leaf)) { Fail-Closed '0007 patched AbstractSymbolCompletionProvider.cs is missing.' }
    ...
    foreach ($requiredProviderText in @(
        ...
        if (-not $symbolProviderText.Contains($requiredProviderText)) { Fail-Closed "0007 request-local receiver cache/provider wiring is missing: $requiredProviderText" }
    }
    ...
    if (-not (Test-Path -LiteralPath $completionServiceTestsSourcePath -PathType Leaf)) { Fail-Closed '0007 CompletionServiceTests.cs is missing.' }
    ...
    foreach ($testName in @(
        ...
        'CompletionSemanticOriginPreservesLexicalAnchorForUnqualifiedCompletion',
        'CompletionSemanticOriginUsesFullChainedReceiverTypeForMemberAccess')) {
        if (-not $completionServiceTestsSourceText.Contains($testName)) { Fail-Closed "0007 completion regression test source is missing: $testName" }
    }
    ...
    Copy-Item -LiteralPath $ReceiverRelativeSemanticOriginPatch -Destination (Join-Path $patchesRoot $CanonicalReceiverRelativeSemanticOriginPatchName)
    ...
    if ($staged0007Hash -ne $receiverRelativeSemanticOriginPatchSha256) { Fail-Closed 'staged 0007 bytes are not byte-identical to the source build-input copy.' }
...
SystemExplorer patched private Roslyn Language Server win-x64 v7
...
LICENSE.txt is preserved byte-for-byte from the verified current V6 Service.ThirdParty.zip and corresponds to pinned Roslyn repository root License.txt (Git blob $ExpectedLicenseBlob).
LICENSE.txt SHA-256:
$($licenseSha256.ToUpperInvariant())

ThirdParty notices provenance:
ThirdPartyNotices.rtf is preserved byte-for-byte from the verified current V6 Service.ThirdParty.zip and corresponds to src/NuGet/ThirdPartyNotices.rtf at the pinned Roslyn commit (Git blob $ExpectedThirdPartyNoticesBlob).
ThirdPartyNotices.rtf SHA-256:
$($thirdPartyNoticesSha256.ToUpperInvariant())

This is a private SystemExplorer production build from the pinned Roslyn source commit plus canonical 0001, canonical 0002, canonical 0003, canonical 0004, canonical 0005, canonical 0006, and canonical 0007.
...
"@
    ...
    $newThirdPartyZipPath = Join-Path $FinalOutputRoot 'Service.ThirdParty_V7.zip'
    ...
    try {
        ...
        if ((Get-ZipEntrySha256Lower $verifyArchive $CanonicalReceiverRelativeSemanticOriginPatchThirdPartyEntry) -ne $receiverRelativeSemanticOriginPatchSha256) { Fail-Closed 'generated ThirdParty 0007 SHA mismatch.' }
...
ServiceThirdPartyV7ZipSha256=$($newThirdPartyZipSha256.ToUpperInvariant())
"@
    ...
    Write-Host "  Service.ThirdParty_V7.zip: $newThirdPartyZipPath"
    ...
    Write-Host "  Service.ThirdParty_V7.zip SHA-256: $newThirdPartyZipSha256"
