...
param(
    ...
    [string] $ImportCompletionReadinessPatch,
    [string] $WorkRoot,
    [string] $OutputRoot,
...
$ExpectedCurrentServiceThirdPartyZipSha256 = '7b079af666741532b5cbcfbfd12c081e743facd17efa7f09b8c869423954dee3'
...
$ExpectedPreparationBaselineDistributionId = 'roslyn-3aeb96c9-systemexplorer-608b4efa8b50-win-x64-v5'
...
$RuntimeArchiveName = 'roslyn-import-completion-readiness-server.zip'
$EvidenceFileName = 'ImportCompletionReadinessProductionRuntimeEvidence.json'
$AdoptionValuesFileName = 'ServiceRuntimeAdoptionValues.txt'
$OwnershipMarkerName = '.systemexplorer-import-completion-readiness-production-owned'
...
function Show-Usage {
    Write-Host 'Usage:'
    Write-Host '  Build-ProductionImportCompletionReadinessRuntime_v1.cmd -RoslynRepositoryRoot <path> -CurrentServiceThirdPartyZip <path> -ImportCompletionReadinessPatch <path> [-WorkRoot <path>] [-OutputRoot <path>] [-DotNetCacheRoot <path>] [-KeepArtifacts]'
    Write-Host ''
    ...
    Write-Host '  SYSTEMEXPLORER_IMPORT_COMPLETION_READINESS_PATCH'
...
if ([string]::IsNullOrWhiteSpace($ImportCompletionReadinessPatch)) { $ImportCompletionReadinessPatch = $env:SYSTEMEXPLORER_IMPORT_COMPLETION_READINESS_PATCH }
if ([string]::IsNullOrWhiteSpace($RoslynRepositoryRoot) -or [string]::IsNullOrWhiteSpace($CurrentServiceThirdPartyZip) -or [string]::IsNullOrWhiteSpace($ImportCompletionReadinessPatch)) {
    [Console]::Error.WriteLine('Roslyn repository, current V5 Service.ThirdParty.zip, and canonical 0006 patch are required via explicit parameters or supported environment variables.')
    Show-Usage
    exit $InvalidArgumentsExitCode
}
...
try {
    ...
    $ImportCompletionReadinessPatch = Get-NormalizedPath $ImportCompletionReadinessPatch
    $shortBaseRoot = Join-Path $env:SystemDrive 'Temp/SECR6'
    ...
    if (-not (Test-Path -LiteralPath $ImportCompletionReadinessPatch -PathType Leaf)) { Fail-Closed "ImportCompletionReadinessPatch does not exist: $ImportCompletionReadinessPatch" }
    ...
    Write-BuildLog "Import-completion readiness production runtime build started. RoslynRepositoryRoot=$RoslynRepositoryRoot"
    Write-BuildLog "CurrentServiceThirdPartyZip=$CurrentServiceThirdPartyZip"
    Write-BuildLog "ImportCompletionReadinessPatch=$ImportCompletionReadinessPatch"
    ...
    if ($currentThirdPartyHash -ne $ExpectedCurrentServiceThirdPartyZipSha256) {
        Fail-Closed "current V5 Service.ThirdParty.zip SHA-256 mismatch; expected=$ExpectedCurrentServiceThirdPartyZipSha256 actual=$currentThirdPartyHash"
    }
    Write-Host "Verified current V5 Service.ThirdParty.zip SHA-256: $currentThirdPartyHash"

    $importCompletionReadinessPatchSha256 = Get-FileSha256Lower $ImportCompletionReadinessPatch
    ...
    $distributionId = "roslyn-3aeb96c9-systemexplorer-$($importCompletionReadinessPatchSha256.Substring(0, 12))-win-x64-v6"
    Write-Host "Canonical 0006 SHA-256: $importCompletionReadinessPatchSha256"
    Write-Host "Production distribution id: $distributionId"

    Assert-UnifiedDiffHunkCounts $ImportCompletionReadinessPatch
    $patchText = [IO.File]::ReadAllText($ImportCompletionReadinessPatch)
    foreach ($requiredText in @(
        'WarmUpForSystemExplorerImportCompletionAsync',
        'WarmUpCacheAsync',
        'OnDiagnosticsComputedAsync',
        ...
        if (-not $patchText.Contains($requiredText)) { Fail-Closed "canonical 0006 is missing required production/regression content: $requiredText" }
    }
    foreach ($forbiddenText in @(
        'diff --git a/src/LanguageServer/Protocol/Handler/DocumentChanges/DidOpenHandler.cs',
        'ExtensionMemberImportCompletionHelper',
        'ForceExpandedCompletionIndexCreation = true',
        'CompletionHandler.GetCompletionListAsync')) {
        if ($patchText.Contains($forbiddenText)) { Fail-Closed "canonical 0006 contains forbidden scope/content: $forbiddenText" }
    }
    ...
    [void](Invoke-GitText $OwnedWorktree @('apply', '--check', '--', $ImportCompletionReadinessPatch))
    [void](Invoke-GitText $OwnedWorktree @('apply', '--', $ImportCompletionReadinessPatch))
    [void](Invoke-GitText $OwnedWorktree @('diff', '--check'))
    Write-Host 'Canonical 0001 -> 0002 -> 0003 -> 0004 -> 0005 -> 0006 patch chain applies cleanly.'
    ...
    try {
        ...
        ) 'Running completion current-source/incremental-reuse regression test class through Visual Studio TestPlatform' $OwnedWorktree
    }
    finally {
    ...
    Copy-Item -LiteralPath $ImportCompletionReadinessPatch -Destination (Join-Path $patchesRoot $CanonicalImportCompletionReadinessPatchName)
    ...
    if ($staged0006Hash -ne $importCompletionReadinessPatchSha256) { Fail-Closed 'staged 0006 bytes are not byte-identical to the source build-input copy.' }
...
SystemExplorer patched private Roslyn Language Server win-x64 v6
...
LICENSE.txt is preserved byte-for-byte from the verified current V5 Service.ThirdParty.zip and corresponds to pinned Roslyn repository root License.txt (Git blob $ExpectedLicenseBlob).
LICENSE.txt SHA-256:
$($licenseSha256.ToUpperInvariant())

ThirdParty notices provenance:
ThirdPartyNotices.rtf is preserved byte-for-byte from the verified current V5 Service.ThirdParty.zip and corresponds to src/NuGet/ThirdPartyNotices.rtf at the pinned Roslyn commit (Git blob $ExpectedThirdPartyNoticesBlob).
ThirdPartyNotices.rtf SHA-256:
$($thirdPartyNoticesSha256.ToUpperInvariant())

This is a private SystemExplorer production build from the pinned Roslyn source commit plus canonical 0001, canonical 0002, canonical 0003, canonical 0004, canonical 0005, and canonical 0006.
...
"@
    ...
    $newThirdPartyZipPath = Join-Path $FinalOutputRoot 'Service.ThirdParty_V6.zip'
    ...
    try {
        ...
        if ((Get-ZipEntrySha256Lower $verifyArchive $CanonicalImportCompletionReadinessPatchThirdPartyEntry) -ne $importCompletionReadinessPatchSha256) { Fail-Closed 'generated ThirdParty 0006 SHA mismatch.' }
...
ServiceThirdPartyV6ZipSha256=$($newThirdPartyZipSha256.ToUpperInvariant())
"@
    ...
    Write-Host "  Service.ThirdParty_V6.zip: $newThirdPartyZipPath"
    ...
    Write-Host "  Service.ThirdParty_V6.zip SHA-256: $newThirdPartyZipSha256"
