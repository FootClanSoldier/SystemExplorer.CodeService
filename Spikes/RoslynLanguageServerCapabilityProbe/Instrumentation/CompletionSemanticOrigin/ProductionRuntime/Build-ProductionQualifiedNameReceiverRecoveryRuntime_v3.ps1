...
param(
    ...
    [string] $QualifiedNameReceiverRecoveryPatch,
    [string] $WorkRoot,
    [string] $OutputRoot,
...
$ExpectedCurrentServiceThirdPartyZipSha256 = 'ad2c4801a8dc06b4d564e46436c2006af7f502e885b5e1df61f767c2c292129f'
...
$ExpectedPreparationBaselineDistributionId = 'roslyn-3aeb96c9-systemexplorer-df87da9cf8f7-win-x64-v8'
...
$RuntimeArchiveName = 'roslyn-qualified-name-receiver-recovery-server.zip'
$EvidenceFileName = 'QualifiedNameReceiverRecoveryProductionRuntimeEvidence.json'
$AdoptionValuesFileName = 'ServiceRuntimeAdoptionValues.txt'
$CompletionTestResultFileName = 'CompletionServiceTestsResult.txt'
$OwnershipMarkerName = '.systemexplorer-qualified-name-receiver-recovery-production-owned'
...
function Show-Usage {
    Write-Host 'Usage:'
    Write-Host '  Build-ProductionQualifiedNameReceiverRecoveryRuntime_v3.cmd -RoslynRepositoryRoot <path> -CurrentServiceThirdPartyZip <path> -QualifiedNameReceiverRecoveryPatch <path> [-WorkRoot <path>] [-OutputRoot <path>] [-DotNetCacheRoot <path>] [-KeepArtifacts]'
    Write-Host ''
    ...
    Write-Host '  SYSTEMEXPLORER_QUALIFIED_NAME_RECEIVER_RECOVERY_PATCH'
...
if ([string]::IsNullOrWhiteSpace($QualifiedNameReceiverRecoveryPatch)) { $QualifiedNameReceiverRecoveryPatch = $env:SYSTEMEXPLORER_QUALIFIED_NAME_RECEIVER_RECOVERY_PATCH }
if ([string]::IsNullOrWhiteSpace($RoslynRepositoryRoot) -or [string]::IsNullOrWhiteSpace($CurrentServiceThirdPartyZip) -or [string]::IsNullOrWhiteSpace($QualifiedNameReceiverRecoveryPatch)) {
    [Console]::Error.WriteLine('Roslyn repository, current V8 Service.ThirdParty.zip, and canonical 0009 patch are required via explicit parameters or supported environment variables.')
    Show-Usage
    exit $InvalidArgumentsExitCode
}
...
try {
    ...
    $QualifiedNameReceiverRecoveryPatch = Get-NormalizedPath $QualifiedNameReceiverRecoveryPatch
    $shortBaseRoot = Join-Path $env:SystemDrive 'Temp/SECR9'
    ...
    if (-not (Test-Path -LiteralPath $QualifiedNameReceiverRecoveryPatch -PathType Leaf)) { Fail-Closed "QualifiedNameReceiverRecoveryPatch does not exist: $QualifiedNameReceiverRecoveryPatch" }
    ...
    Write-BuildLog "Qualified-name receiver-recovery production runtime build started. RoslynRepositoryRoot=$RoslynRepositoryRoot"
    Write-BuildLog "CurrentServiceThirdPartyZip=$CurrentServiceThirdPartyZip"
    Write-BuildLog "QualifiedNameReceiverRecoveryPatch=$QualifiedNameReceiverRecoveryPatch"
    ...
    if ($currentThirdPartyHash -ne $ExpectedCurrentServiceThirdPartyZipSha256) {
        Fail-Closed "current V8 Service.ThirdParty.zip SHA-256 mismatch; expected=$ExpectedCurrentServiceThirdPartyZipSha256 actual=$currentThirdPartyHash"
    }
    Write-Host "Verified current V8 Service.ThirdParty.zip SHA-256: $currentThirdPartyHash"

    $qualifiedNameReceiverRecoveryPatchSha256 = Get-FileSha256Lower $QualifiedNameReceiverRecoveryPatch
    ...
    $distributionId = "roslyn-3aeb96c9-systemexplorer-$($qualifiedNameReceiverRecoveryPatchSha256.Substring(0, 12))-win-x64-v9"
    Write-Host "Canonical 0009 SHA-256: $qualifiedNameReceiverRecoveryPatchSha256"
    Write-Host "Production distribution id: $distributionId"

    Assert-UnifiedDiffHunkCounts $QualifiedNameReceiverRecoveryPatch
    $patchText = [IO.File]::ReadAllText($QualifiedNameReceiverRecoveryPatch)
    ...
    foreach ($requiredText in @(
        ...
        if (-not $patchText.Contains($requiredText)) { Fail-Closed "canonical 0009 is missing required qualified-name recovery/regression content: $requiredText" }
    }
    foreach ($forbiddenText in @(
        ...
        'PROVENANCE.txt')) {
        if ($patchText.Contains($forbiddenText)) { Fail-Closed "canonical 0009 contains forbidden scope/content: $forbiddenText" }
    }
    ...
    [void](Invoke-GitText $OwnedWorktree @('apply', '--check', '--whitespace=error-all', '--', $QualifiedNameReceiverRecoveryPatch))
    [void](Invoke-GitText $OwnedWorktree @('apply', '--whitespace=error-all', '--', $QualifiedNameReceiverRecoveryPatch))
    [void](Invoke-GitText $OwnedWorktree @('diff', '--check'))
    Write-Host 'Canonical 0001 -> 0002 -> 0003 -> 0004 -> 0005 -> 0006 -> 0007 -> 0008 -> 0009 patch chain applies cleanly.'
    ...
    if (-not (Test-Path -LiteralPath $symbolProviderPath -PathType Leaf)) { Fail-Closed '0007-patched AbstractSymbolCompletionProvider.cs is missing after 0009.' }
    ...
    foreach ($requiredProviderText in @(
        ...
        if (-not $symbolProviderText.Contains($requiredProviderText)) { Fail-Closed "0007 request-local receiver cache/provider wiring regressed after 0009: $requiredProviderText" }
    }
    ...
    if (-not $completionServiceTestsSourceText.Contains('AssertSystemExplorerCompletionEvidenceWithoutDepth')) {
        Fail-Closed '0008 namespace fail-closed no-depth assertion helper is missing after 0009.'
    }
    ...
    Invoke-NativeLogged $roslynDotNetHost @(
        ...
        '--filter', 'FullyQualifiedName~TestSystemExplorerImportCompletion|FullyQualifiedName~TestDoNotProvideOverrideTextEditsOrInsertTextAsync'
    ) 'Running 0005/0006 SystemExplorer import-completion readiness and complex-edit regression tests' $OwnedWorktree
    ...
    Copy-Item -LiteralPath $QualifiedNameReceiverRecoveryPatch -Destination (Join-Path $patchesRoot $CanonicalQualifiedNameReceiverRecoveryPatchName)
    ...
    if ($staged0009Hash -ne $qualifiedNameReceiverRecoveryPatchSha256) { Fail-Closed 'staged 0009 bytes are not byte-identical to the source build-input copy.' }
...
SystemExplorer patched private Roslyn Language Server win-x64 v9
...
LICENSE.txt is preserved byte-for-byte from the verified current V8 Service.ThirdParty.zip and corresponds to pinned Roslyn repository root License.txt (Git blob $ExpectedLicenseBlob).
LICENSE.txt SHA-256:
$($licenseSha256.ToUpperInvariant())

ThirdParty notices provenance:
ThirdPartyNotices.rtf is preserved byte-for-byte from the verified current V8 Service.ThirdParty.zip and corresponds to src/NuGet/ThirdPartyNotices.rtf at the pinned Roslyn commit (Git blob $ExpectedThirdPartyNoticesBlob).
ThirdPartyNotices.rtf SHA-256:
$($thirdPartyNoticesSha256.ToUpperInvariant())

This is a private SystemExplorer production build from the pinned Roslyn source commit plus canonical 0001, canonical 0002, canonical 0003, canonical 0004, canonical 0005, canonical 0006, canonical 0007, canonical 0008, and canonical 0009.
...
"@
    ...
    $newThirdPartyZipPath = Join-Path $FinalOutputRoot 'Service.ThirdParty_V9.zip'
    ...
    try {
        ...
        if ((Get-ZipEntrySha256Lower $verifyArchive $CanonicalQualifiedNameReceiverRecoveryPatchThirdPartyEntry) -ne $qualifiedNameReceiverRecoveryPatchSha256) { Fail-Closed 'generated ThirdParty 0009 SHA mismatch.' }
...
ServiceThirdPartyV9ZipSha256=$($newThirdPartyZipSha256.ToUpperInvariant())
"@
...
"@
    ...
    Write-Host "  Service.ThirdParty_V9.zip: $newThirdPartyZipPath"
    ...
    Write-Host "  Service.ThirdParty_V9.zip SHA-256: $newThirdPartyZipSha256"
