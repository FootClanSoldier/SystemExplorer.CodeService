...
param(
    ...
    [string] $ImportCompletionContractPatch,
    [string] $WorkRoot,
    [string] $OutputRoot,
...
$ExpectedCurrentServiceThirdPartyZipSha256 = '8d418b18c2228acf64a38a1282d2a1e3a029f057ee66ece0d20242f14d6b8112'
...
$ExpectedPreparationBaselineDistributionId = 'roslyn-3aeb96c9-systemexplorer-39d4217634dc-win-x64-v4'
...
$RuntimeArchiveName = 'roslyn-import-completion-contract-server.zip'
$EvidenceFileName = 'ImportCompletionContractProductionRuntimeEvidence.json'
$AdoptionValuesFileName = 'ServiceRuntimeAdoptionValues.txt'
$OwnershipMarkerName = '.systemexplorer-import-completion-contract-production-owned'
...
function Show-Usage {
    Write-Host 'Usage:'
    Write-Host '  Build-ProductionImportCompletionContractRuntime_v1.cmd -RoslynRepositoryRoot <path> -CurrentServiceThirdPartyZip <path> -ImportCompletionContractPatch <path> [-WorkRoot <path>] [-OutputRoot <path>] [-DotNetCacheRoot <path>] [-KeepArtifacts]'
    Write-Host ''
    ...
    Write-Host '  SYSTEMEXPLORER_IMPORT_COMPLETION_CONTRACT_PATCH'
...
if ([string]::IsNullOrWhiteSpace($ImportCompletionContractPatch)) { $ImportCompletionContractPatch = $env:SYSTEMEXPLORER_IMPORT_COMPLETION_CONTRACT_PATCH }
if ([string]::IsNullOrWhiteSpace($RoslynRepositoryRoot) -or [string]::IsNullOrWhiteSpace($CurrentServiceThirdPartyZip) -or [string]::IsNullOrWhiteSpace($ImportCompletionContractPatch)) {
    [Console]::Error.WriteLine('Roslyn repository, current V4 Service.ThirdParty.zip, and canonical 0005 patch are required via explicit parameters or supported environment variables.')
    Show-Usage
    exit $InvalidArgumentsExitCode
}
...
try {
    ...
    $ImportCompletionContractPatch = Get-NormalizedPath $ImportCompletionContractPatch
    $shortBaseRoot = Join-Path $env:SystemDrive 'Temp/SECR5'
    if ([string]::IsNullOrWhiteSpace($DotNetCacheRoot)) { $DotNetCacheRoot = $env:SYSTEMEXPLORER_ROSLYN_DOTNET_CACHE_ROOT }
    if ([string]::IsNullOrWhiteSpace($DotNetCacheRoot)) { $DotNetCacheRoot = Join-Path $shortBaseRoot 'cache/dotnet' }
    ...
    if (-not (Test-Path -LiteralPath $ImportCompletionContractPatch -PathType Leaf)) { Fail-Closed "ImportCompletionContractPatch does not exist: $ImportCompletionContractPatch" }
    ...
    Write-BuildLog "Import-completion contract production runtime build started. RoslynRepositoryRoot=$RoslynRepositoryRoot"
    Write-BuildLog "CurrentServiceThirdPartyZip=$CurrentServiceThirdPartyZip"
    Write-BuildLog "ImportCompletionContractPatch=$ImportCompletionContractPatch"
    ...
    if ($currentThirdPartyHash -ne $ExpectedCurrentServiceThirdPartyZipSha256) {
        Fail-Closed "current V4 Service.ThirdParty.zip SHA-256 mismatch; expected=$ExpectedCurrentServiceThirdPartyZipSha256 actual=$currentThirdPartyHash"
    }
    Write-Host "Verified current V4 Service.ThirdParty.zip SHA-256: $currentThirdPartyHash"

    $importCompletionContractPatchSha256 = Get-FileSha256Lower $ImportCompletionContractPatch
    ...
    $distributionId = "roslyn-3aeb96c9-systemexplorer-$($importCompletionContractPatchSha256.Substring(0, 12))-win-x64-v5"
    Write-Host "Canonical 0005 SHA-256: $importCompletionContractPatchSha256"
    Write-Host "Production distribution id: $distributionId"

    Assert-UnifiedDiffHunkCounts $ImportCompletionContractPatch
    $patchText = [IO.File]::ReadAllText($ImportCompletionContractPatch)
    foreach ($requiredText in @(
        '_systemExplorer_supportsImportCompletion',
        'SystemExplorerSupportsImportCompletion',
        'SupportSystemExplorerImportCompletion',
        '_systemExplorer_completionRequiresImport',
        'SystemExplorerCompletionRequiresImport',
        'item.IsComplexTextEdit && item.Flags.IsExpanded()',
        ...
        if (-not $patchText.Contains($requiredText)) { Fail-Closed "canonical 0005 is missing required production/regression content: $requiredText" }
    }
    ...
    [void](Invoke-GitText $OwnedWorktree @('apply', '--check', '--', $ImportCompletionContractPatch))
    [void](Invoke-GitText $OwnedWorktree @('apply', '--', $ImportCompletionContractPatch))
    [void](Invoke-GitText $OwnedWorktree @('diff', '--check'))
    Write-Host 'Canonical 0001 -> 0002 -> 0003 -> 0004 -> 0005 patch chain applies cleanly.'
    ...
    ) 'Running 0005 VS-internal import-completion and complex-edit regression tests' $OwnedWorktree
    ...
    Copy-Item -LiteralPath $ImportCompletionContractPatch -Destination (Join-Path $patchesRoot $CanonicalImportCompletionContractPatchName)
    ...
    if ($staged0005Hash -ne $importCompletionContractPatchSha256) { Fail-Closed 'staged 0005 bytes are not byte-identical to the source build-input copy.' }
...
SystemExplorer patched private Roslyn Language Server win-x64 v5
...
LICENSE.txt is preserved byte-for-byte from the verified current V4 Service.ThirdParty.zip and corresponds to pinned Roslyn repository root License.txt (Git blob $ExpectedLicenseBlob).
LICENSE.txt SHA-256:
$($licenseSha256.ToUpperInvariant())

ThirdParty notices provenance:
ThirdPartyNotices.rtf is preserved byte-for-byte from the verified current V4 Service.ThirdParty.zip and corresponds to src/NuGet/ThirdPartyNotices.rtf at the pinned Roslyn commit (Git blob $ExpectedThirdPartyNoticesBlob).
ThirdPartyNotices.rtf SHA-256:
$($thirdPartyNoticesSha256.ToUpperInvariant())

This is a private SystemExplorer production build from the pinned Roslyn source commit plus canonical 0001, canonical 0002, canonical 0003, canonical 0004, and canonical 0005.
...
"@
    ...
    $newThirdPartyZipPath = Join-Path $FinalOutputRoot 'Service.ThirdParty_V5.zip'
    ...
    try {
        ...
        if ((Get-ZipEntrySha256Lower $verifyArchive $CanonicalImportCompletionContractPatchThirdPartyEntry) -ne $importCompletionContractPatchSha256) { Fail-Closed 'generated ThirdParty 0005 SHA mismatch.' }
...
ServiceThirdPartyV5ZipSha256=$($newThirdPartyZipSha256.ToUpperInvariant())
"@
    ...
    Write-Host "  Service.ThirdParty_V5.zip: $newThirdPartyZipPath"
    ...
    Write-Host "  Service.ThirdParty_V5.zip SHA-256: $newThirdPartyZipSha256"
